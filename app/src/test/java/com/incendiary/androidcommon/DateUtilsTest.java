...
public class DateUtilsTest {
  ...
  private void testGetDayDifferenceOfReadableIntsPlusMinusNDays(int sign) {
    ...
    Dates.setTime(calendar, 12, 0, 0, 0);
    int today = Dates.getDayAsReadableInt(calendar);
    for (int i = 1; i <= 5000; i++) {
      Dates.addDays(calendar, sign * 1);
      int day = Dates.getDayAsReadableInt(calendar);
      int diff = Dates.getDayDifferenceOfReadableInts(today, day);
  ...
  private void checkDayDifference(int day1, int day2, int expectedDifference) {
    int actualDifference = Dates.getDayDifferenceOfReadableInts(day1, day2);
